...
Authentication, when needed, is supplied as a token in the context: the module's
`git` wrapper injects it as an HTTP Authorization header, exactly as the previous
Azure DevOps path did.

.PARAMETER Context
A hashtable. Recognized keys:
  Url      - the git remote to clone (required).
  Token    - optional credential (PAT / JIT) for git-over-HTTPS auth.
...
# The module's `git` wrapper reads $JITToken from the calling scope to build the
# HTTP Authorization header, so expose the supplied token under that name.
if (-not [string]::IsNullOrWhiteSpace($Context.Token)) {
    $JITToken = $Context.Token
}
